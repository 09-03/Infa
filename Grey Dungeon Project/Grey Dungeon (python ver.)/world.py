...
@njit(fastmath = True, cache = True)
def ray_func(player_pos, player_angle, world_map):
    casted_walls = []
    x0, y0 = player_pos
    texture_vertical, texture_horizontal = 1, 1
    x_mapped, y_mapped = mapping(x0, y0)
    ...
    for ray in range(300):
        ...
        x, dx = (x_mapped + 100, 1) if cos_a >= 0 else (x_mapped, -1)
        for i in range(0, world_width, 100):
            depth_vertical = (x - x0) / cos_a
            yv = y0 + depth_vertical * sin_a
        ...
        y, dy = (y_mapped + 100, 1) if sin_a >= 0 else (y_mapped, -1)
        for i in range(0, world_height, 100):
            depth_horizontal = (y - y0) / sin_a
            xh = x0 + depth_horizontal * cos_a
        ...
        depth, offset, texture = (depth_vertical, yv, texture_vertical) if depth_vertical < depth_horizontal else (depth_horizontal, xh, texture_horizontal)
...
@njit(fastmath=True, cache=True)
def ray_npc_2_player_func(npc_x, npc_y, world_map, player_pos):
    x0, y0 = player_pos
    x_mapped, y_mapped = mapping(x0, y0)
...
def ray_2_walls_func(player, textures):
    casted_walls = ray_func(player.pos, player.angle, world_map)
...
class Graphics:
    ...
    def world(self, world_objects):
        for obj in sorted(world_objects, key=lambda n: n[0], reverse=True):
            if obj[0]:
                empty_var, object, object_pos = obj
...
class SpriteObject:
    ...
    def object_locate(self, player):
        ...
        if 0 <= fake_ray <= 499 and self.distance_to_sprite > 30:
            ...
            shift = (sprite_height // 2) * self.shift
            if self.is_dead and self.is_dead != "immortal":
                sprite_object = self.dead_animation()
                shift = (sprite_height // 2) * self.dead_shift
            ...
            sprite_pos = (self.current_ray * 4 - sprite_width // 2, 400 - sprite_height // 2 + shift)
...
class Interaction:
    def __init__(self, player, player2, sprites, graphics):
        self.player = player
    ...
    def interaction_objects(self):
        if self.player.shot and self.graphics.shot_animation_trigger:
            for obj in sorted(self.sprites.list_of_objects, key=lambda obj: obj.distance_to_sprite):
                if obj.is_on_fire[1]:
                    if obj.is_dead != "immortal" and not obj.is_dead:
                        if ray_npc_2_player_func(obj.x, obj.y, world_map, self.player.pos):
                            if obj.flag == "npc":
                            ...
                            self.graphics.shot_animation_trigger = False
                    break
