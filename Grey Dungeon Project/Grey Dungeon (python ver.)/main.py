...
graphics = Graphics(sc, player, clock)
interaction = Interaction(player, player2, sprites, graphics)
...
while True:
    ...
    graphics.background(player.angle)
    walls, wall_shot = ray_2_walls_func(player, graphics.textures)
    graphics.world(walls + [obj.object_locate(player) for obj in sprites.list_of_objects])
    graphics.fps(clock)
    graphics.player_weapon([wall_shot, sprites.sprite_shot])
    ...
    sync(synced)
    graphics.health_bar(player, player2)
    ...
    if (player2.health + player.enemy_health) // 2 <= 0 or (solo and player.dead_sprite == 1):
        tick += 1
        if tick == 100:
            win = True
            interaction.play_endmusic(win)
